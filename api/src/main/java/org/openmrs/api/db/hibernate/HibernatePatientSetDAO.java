...
public class HibernatePatientSetDAO implements PatientSetDAO {
	...
	private String formatUserName(Person u) {
		return u.getPersonName().toString();
	}
	
	private String formatUser(Person u) {
		StringBuilder ret = new StringBuilder();
		ret.append(u.getId() + "^" + formatUserName(u));
		return ret.toString();
	}
		...
		try {
			...
			for (Encounter e : encounters) {
				...
				Element metadataNode = doc.createElement("metadata");
				{
					...
					Provider provider = (Provider) e.getActiveEncounterProviders().toArray()[0];
					Person u = provider.getPerson();
					if (u != null) {
				...
				if (orders != null && orders.size() != 0) {
					...
					for (Order order : orders) {
						...
						if (order.getEffectiveStartDate() != null) {
							orderNode.setAttribute("start_date", df.format(order.getEffectiveStartDate()));
						}
						if (order.getAutoExpireDate() != null) {
							orderNode.setAttribute("auto_expire_date", df.format(order.getAutoExpireDate()));
						}
						if (order.getOrderer() != null) {
							orderNode.setAttribute("orderer", formatUser(order.getOrderer().getPerson()));
						}
						if (order.getVoided() != null) {
							orderNode.setAttribute("discontinued", order.getVoided().toString());
						}
						if (order.getDateVoided() != null) {
							orderNode.setAttribute("discontinued_date", df.format(order.getDateVoided()));
						}
						if (order.getVoidReason() != null) {
							orderNode.setAttribute("discontinued_reason", order.getVoidReason());
						}
	...
	public Cohort getPatientsByRelationship(RelationshipType relType, boolean includeAtoB, boolean includeBtoA, Person target) {
		...
		if (relType != null) {
			if (includeAtoB && includeBtoA) {
				...
				CohortMembership cmship = new CohortMembership();
				cmship.setPatientId(target.getPersonId());
				ret.removeMembership(cmship) ; 
